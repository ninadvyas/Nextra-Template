---
type: page
title: Projects
date: 2021-03-18
---

# Projects

<h1 className='title1'>Here's My Projects</h1>

import Image from 'next/image'
import {

  FaLinkedin,FaLink
 
} from 'react-icons/fa'
import {

FiExternalLink
 
} from 'react-icons/fi'
import {

HiLink
 
} from 'react-icons/hi'

{/* const cursor = document.qureySelector('.cursor');
document.addEventListener('mousemove',(e) => {
  cursor.style.left = e.pageX + 'px';
   cursor.style.top = e.pageX + 'px';
}) */}

<div className="testimonials-container">

{' '}
<div className="testimonial-item4">

<div className='sectionp'>
  <header className='prtitle'>Crypto Base</header>
  <header className='prspan'>A platform where you can save your favorite coin & see the volumes,prices.</header>


    <span className='prlink'>📁 CheckOut!! <a  href='https://github.com/NinadVyas/WebCryptoBase'><FiExternalLink className='icon'/> </a></span>
  
  </div>
</div>


<div className="testimonial-item3">
<div className='sectionp'>
  <header className='prtitle'>Room Realm </header>
  <header className='prspan'>A chat room where everybody can talk about anything!</header>


    <span className='prlink'>📁 CheckOut!!<a  href='https://github.com/NinadVyas/Roomrealm'> <FiExternalLink className='icon'/> </a></span>
  
  </div>
</div>


<div className="testimonial-item2">
<div className='sectionp'>
  <header className='prtitle'>College Space</header>
  <header className='prspan'>A platform that provides docs and projects to learn</header>


    <span className='prlink'>📁 CheckOut!!  <a  href='https://github.com/NinadVyas/Collage-Space'> <FiExternalLink className='icon'/> </a></span>
  
  </div>
</div>


<div className="testimonial-item1">
<div className='sectionp'>
  <header className='prtitle'>Quiz Web</header>
  <header className='prspan'>A Web App in which user can take quiz and see result instantly</header>


    <span className='prlink'>📁 CheckOut!!  <a  href='https://github.com/NinadVyas/quizweb'> <FiExternalLink className='icon'/> </a></span>
  
  </div>
</div>

{/* <div className='cursor'></div> */}

</div>
<div >
  <a className='btmmore' href='https://github.com/NinadVyas'> <p className='btmmore'>More on Github ↗</p> </a>
</div>
---